		...
		_create	: function() {
			...
			var input = this.input = $("<input>").insertAfter(select).val(value).autocomplete({
				...
				source		: function(request, response) {
					...
					result= result && result.size()>20 ? result.slice(0,20):result;
					response(result);
				},
				select		: function(event, ui) {
					...
					var parentSel$=$(ui.item.option.parentElement);
					parentSel$.val(ui.item.option.value);
					parentSel$.trigger('change',event);  //move to input.blur
				},
			...
			select.data("combox-cascading",input);
